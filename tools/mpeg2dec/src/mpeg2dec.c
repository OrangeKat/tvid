...
static void decode_mpeg2 (uint8_t * current, uint8_t * end)
{
	...
	case STATE_INVALID_END:
		...
		if (info->display_picture)
		    fprintf (stderr, "progressive_frame=%d top_field_first=%d repeat_first_field=%d\n",
			     (info->display_picture->flags & PIC_FLAG_PROGRESSIVE_FRAME) ? 1 : 0,
			     (info->display_picture->flags & PIC_FLAG_TOP_FIELD_FIRST) ? 1 : 0,
			     (info->display_picture->flags & PIC_FLAG_REPEAT_FIRST_FIELD) ? 1 : 0);
		if (output->draw)
